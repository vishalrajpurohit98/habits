...
var uaiMode='ask';
function uaiModeInstruction(){
  if(uaiMode==='plan') return 'MODE: PLAN. Help organize the user’s day/week using real tracker data. For “plan my day”, propose a practical ordered plan using due tasks, priorities, habits, and known commitments. Do not invent calendar events. Ask one question only if a key planning constraint is genuinely missing. Do not modify data merely by proposing a plan.';
  if(uaiMode==='analyze') return 'MODE: ANALYZE. Focus on patterns, trends, bottlenecks, consistency, aging, spending, sleep, mood, workouts and habit performance. Use actual APP DATA numbers. Explain what matters and give concise actionable recommendations. Do not mutate data unless the user explicitly asks.';
  if(uaiMode==='act') return 'MODE: ACT. The user wants the assistant to make a tracker change when sufficiently specified. Collect missing required fields one at a time, then execute. For destructive or bulk changes, require confirmation.';
  return 'MODE: ASK. Answer questions naturally and help the user complete tracker actions conversationally.';
}
function uaiPrompt(text,conversationContext){
  return 'You are the universal AI assistant for Personal Tracker. '+uaiModeInstruction()+'\n\n'
  +'You can READ/CREATE/UPDATE/DELETE across the app: Habits, Tasks, Expenses/Transactions, Mood, Sleep, Journal, Workouts/Exercises, Settings and Navigation.\n\n'
...
function executeAction(r){
  try{
    ...
    if(a==='add_journal'){
      var je={id:Date.now().toString(36)+Math.random().toString(36).slice(2,7),d:p.date||today(),t:String(p.title||'').trim().slice(0,120),b:String(p.body||'').trim().slice(0,5000),ph:[],created:Date.now()};
      if(!je.t&&!je.b)return{ok:0,msg:'Enter a journal title or body.'};state.jr.push(je);sortJr();persist();if($('pgJr')&&$('pgJr').classList.contains('on'))renderJr();return{ok:1,msg:msg,detail:'Journal entry saved for '+je.d};
    }
    if(a==='update_journal'){
      var je2=p.id?jrFind(p.id):null;if(!je2&&p.title){je2=state.jr.find(function(x){return String(x.t||'').toLowerCase()===String(p.title).toLowerCase();})||null;}if(!je2)return{ok:0,msg:'Journal entry not found.'};var oldJ=JSON.parse(JSON.stringify(je2));if(p.date)je2.d=p.date;if(p.title!==undefined)je2.t=String(p.title).trim().slice(0,120);if(p.body!==undefined)je2.b=String(p.body).trim().slice(0,5000);sortJr();persist();if($('pgJr')&&$('pgJr').classList.contains('on'))renderJr();return{ok:1,msg:msg,detail:'Journal entry updated',undo:function(){var ix=state.jr.findIndex(function(x){return x.id===oldJ.id;});if(ix>=0)state.jr[ix]=oldJ;persist();}};
    }
    if(a==='delete_journal'){
      var jf=state.jr.filter(function(x){return (!p.id||x.id===p.id)&&(!p.date||x.d===p.date)&&(!p.title||String(x.t||'').toLowerCase().indexOf(String(p.title).toLowerCase())>=0);});if(!jf.length)return{ok:0,msg:'Journal entry not found.'};if(jf.length>1)return{ok:0,msg:'I found multiple journal entries. Specify the title or date.'};var jd=jf[0];if(!r.confirm)return{ok:0,msg:'Please confirm deleting journal entry “'+(jd.t||'Untitled')+'”.',needConfirm:1,pending:{action:'delete_journal',params:{id:jd.id},confirm:true}};state.jr=state.jr.filter(function(x){return x.id!==jd.id;});persist();if($('pgJr')&&$('pgJr').classList.contains('on'))renderJr();return{ok:1,msg:msg,detail:'Deleted journal entry',undo:function(){state.jr.push(jd);persist();}};
    }
    if(a==='log_workout'){
...
function uaiSuggestedFollowups(result){
  ...
  if(a==='query')return uaiMode==='plan'?['Plan my whole week','What should I prioritize?']:uaiMode==='analyze'?['Show the biggest problem','How can I improve this?']:['What should I focus on next?','Give me a short summary'];
...
function init(){
  ...
  // ---- AI Chat voice input: native Android SpeechRecognizer with ~2.5s silence finalization; browser fallback ----
  var uaiSpeech={active:false,recognition:null,silenceTimer:null};
  function setUaiListening(on,msg){
    var b=$('uaiMic'),st=$('uaiListenStatus'),tx=$('uaiListenText');
    if(b)b.classList.toggle('listening',!!on);
    if(st)st.classList.toggle('on',!!on);
    if(tx&&msg)tx.textContent=msg;
    if(b){b.textContent=on?'■':'🎙️';b.title=on?'Stop listening':'Speak';}
  }
  function finishUaiSpeech(text){
    text=String(text||'').trim(); if(!text){setUaiListening(false,'');return;}
    uaiSpeech.active=false; setUaiListening(false,'');
    $('uaiInput').value=text;
    doUaiSend();
  }
  function stopUaiSpeech(){
    uaiSpeech.active=false;
    if(uaiSpeech.silenceTimer){clearTimeout(uaiSpeech.silenceTimer);uaiSpeech.silenceTimer=null;}
    try{if(window.Bridge&&Bridge.stopAiSpeech)Bridge.stopAiSpeech();}catch(e){}
    try{if(uaiSpeech.recognition){uaiSpeech.recognition.stop();}}catch(e){}
    uaiSpeech.recognition=null; setUaiListening(false,'');
  }
  function startBrowserUaiSpeech(){
    var SR=window.SpeechRecognition||window.webkitSpeechRecognition;
    if(!SR){setUaiListening(false,'');toastN('Voice input is not available on this device.');return;}
    try{if(uaiSpeech.recognition)uaiSpeech.recognition.stop();}catch(e){}
    var r=new SR();uaiSpeech.recognition=r;uaiSpeech.active=true;r.continuous=false;r.interimResults=true;r.lang='en-IN';
    var last='';
    r.onstart=function(){setUaiListening(true,'Listening… keep speaking; I’ll finalize after about 2.5 seconds of silence.');};
    r.onresult=function(ev){var finalText='';for(var i=ev.resultIndex;i<ev.results.length;i++){var tx=ev.results[i][0].transcript||'';if(ev.results[i].isFinal)finalText+=tx;else last=tx;}var shown=(finalText||last).trim();if(shown)$('uaiInput').value=shown;if(finalText){if(uaiSpeech.silenceTimer)clearTimeout(uaiSpeech.silenceTimer);uaiSpeech.silenceTimer=setTimeout(function(){finishUaiSpeech(finalText);},2500);}};
    r.onerror=function(){if(uaiSpeech.active){setUaiListening(false,'');toastN('Could not capture voice input. Please try again.');}};
    r.onend=function(){if(uaiSpeech.active&&last){if(uaiSpeech.silenceTimer)clearTimeout(uaiSpeech.silenceTimer);uaiSpeech.silenceTimer=setTimeout(function(){finishUaiSpeech($('uaiInput').value);},2500);}};
    try{r.start();}catch(e){uaiSpeech.active=false;setUaiListening(false,'');toastN('Could not start voice input.');}
  }
  window._aiSpeechState=function(state,msg){if(state==='listening'||state==='starting'||state==='ready'){uaiSpeech.active=true;setUaiListening(true,msg||'Listening… I’ll wait for about 2.5 seconds of silence before sending.');}else if(state==='processing'){setUaiListening(true,'Finishing your speech…');}};
  window._aiSpeechPartial=function(text){if(uaiSpeech.active&&String(text||'').trim())$('uaiInput').value=String(text).trim();};
  window._aiSpeechResult=function(text){finishUaiSpeech(text);};
  window._aiSpeechError=function(msg){uaiSpeech.active=false;setUaiListening(false,'');toastN(String(msg||'Voice input failed.'));};
  function startUaiSpeech(){
    if(uaiSpeech.active){stopUaiSpeech();return;}
    if(window.Bridge&&Bridge.startAiSpeech){try{uaiSpeech.active=true;setUaiListening(true,'Listening… keep speaking; I’ll finalize after about 2.5 seconds of silence.');Bridge.startAiSpeech();return;}catch(e){uaiSpeech.active=false;}}
    startBrowserUaiSpeech();
  }

  // ---- Ask Your Data chat ----
  // ---- Universal AI Assistant wiring ----
  function doUaiSend(){ var v=$('uaiInput').value.trim();if(!v)return;stopUaiSpeech();$('uaiInput').value='';uaiSend(v); }
  $('uaiMic').addEventListener('click',function(){startUaiSpeech();});
  document.querySelectorAll('.uaiMode').forEach(function(b){b.addEventListener('click',function(){uaiMode=this.getAttribute('data-mode')||'ask';document.querySelectorAll('.uaiMode').forEach(function(x){x.classList.toggle('sel',x===b);});var hints={ask:'Ask anything about your tracker.',plan:'Plan my day, organize priorities, or build a practical week.',analyze:'Find patterns, trends, bottlenecks, spending or consistency issues.',act:'Make changes to my tracker; I’ll collect missing details safely.'};$('uaiModeHint').textContent=hints[uaiMode]||hints.ask;$('uaiInput').focus();});});
