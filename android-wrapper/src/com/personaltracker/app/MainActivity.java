...
public class MainActivity extends Activity {
    ...
    void configureWebView(WebView w) {
        ...
        w.setDownloadListener((url, userAgent, contentDisposition, mimeType, contentLength) -> {
            if (url.startsWith("blob:")) {
                w.evaluateJavascript("(function(){var x=new XMLHttpRequest();x.open('GET','" + url + "',true);x.responseType='blob';x.onload=function(){var r=new FileReader();r.onloadend=function(){try{Bridge.saveDataUrl(r.result,'" + mimeType + "')}catch(e){}};r.readAsDataURL(x.response);};x.send();})()", null);
            } else {
                try {
    ...
    @Override protected void onNewIntent(Intent intent) { super.onNewIntent(intent); setIntent(intent); handleLaunchIntent(intent); }
    ...
    void handleLaunchIntent(Intent intent) {
        ...
        if (habit != null && !habit.isEmpty()) {
            web.postDelayed(() -> web.evaluateJavascript(
                    "if(typeof openDetail==='function'){showTab('pgToday');openDetail(" + JSONObject.quote(habit) + ")}", null), 400);
        }
    }
    ...
    void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= 26) {
            ...
            c.setDescription("Scheduled habit reminders");
    ...
    boolean bioAvailable() {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= 29) {
                // Use PackageManager to check for biometric hardware
    ...
    void showBiometricPrompt() {
        ...
        try {
            BiometricPrompt.Builder builder = new BiometricPrompt.Builder(this)
                    ...
                    .setDescription("Use biometrics to unlock")
                    .setNegativeButton("Use PIN", getMainExecutor(), (d, w) -> js("window.bioResult&&window.bioResult(false)"));
            if (Build.VERSION.SDK_INT >= 30) {
                builder.setAllowedAuthenticators(
                        android.hardware.biometrics.BiometricManager.Authenticators.BIOMETRIC_WEAK);
            }
            builder.build().authenticate(new CancellationSignal(),
                    getMainExecutor(), new BiometricPrompt.AuthenticationCallback() {
                        @Override public void onAuthenticationSucceeded(BiometricPrompt.AuthenticationResult result) {
                            js("window.bioResult&&window.bioResult(true)");
                        }
                        @Override public void onAuthenticationError(int errorCode, CharSequence errString) {
                            js("window.bioResult&&window.bioResult(false)");
                        }
                        @Override public void onAuthenticationFailed() { /* retry automatically */ }
                    });
        ...
        if (req == REQ_IMPORT) {
            ...
            try {
                ...
                try (InputStream in = getContentResolver().openInputStream(data.getData());
                     OutputStream out = new FileOutputStream(f)) { byte[] buf = new byte[8192]; int n; while ((n = in.read(buf)) > 0) out.write(buf, 0, n); }
    ...
    void shareFile(String name, String mime, String b64) throws Exception {
        ...
        Uri u = NativeFileProvider.getUriForFile(this, getPackageName() + ".fileprovider", f);
    ...
    public class Bridge {
        ...
        @JavascriptInterface public String appVer() { return "3.5"; }
        ...
        @JavascriptInterface public String getLaunchAction() { Intent i = getIntent(); JSONObject o = new JSONObject(); try { if (i != null) { if (i.getStringExtra("habit") != null) o.put("habit", i.getStringExtra("habit")); if (i.getStringExtra("tab") != null) o.put("tab", i.getStringExtra("tab")); if (i.getStringExtra("date") != null) o.put("date", i.getStringExtra("date")); if (i.getStringExtra("type") != null) o.put("type", i.getStringExtra("type")); } } catch (Exception ignored) {} return o.toString(); }
        @JavascriptInterface public boolean bioAvail() { return bioAvailable(); }
        @JavascriptInterface public void bio() { runOnUiThread(() -> showBiometricPrompt()); }
        ...
        @JavascriptInterface public void saveDataUrl(String dataUrl, String mime) {
            try {
                int comma = dataUrl.indexOf(','); String b64 = dataUrl.substring(comma + 1);
                byte[] bytes = Base64.decode(b64, Base64.DEFAULT);
                String ext = "bin"; if (mime != null) { if (mime.contains("spreadsheet") || mime.contains("excel")) ext = "xlsx"; else if (mime.contains("pdf")) ext = "pdf"; else if (mime.contains("json")) ext = "json"; }
                String name = "PersonalTracker-" + System.currentTimeMillis() + "." + ext;
