...
public class MainActivity extends Activity {
    ...
    final Executor bioExecutor = command -> runOnUiThread(command);
            ...
            runOnUiThread(() -> {
                try {
                    BiometricPrompt prompt = new BiometricPrompt.Builder(MainActivity.this)
                        ...
                        .setNegativeButton("Use PIN", getMainExecutor(), (dialog, which) -> js("window.bioResult&&window.bioResult(false)"))
                        .build();
                    prompt.authenticate(new BiometricPrompt.AuthenticationCallback(){
                        @Override public void onAuthenticationSucceeded(BiometricPrompt.AuthenticationResult result){ js("window.bioResult&&window.bioResult(true)"); }
                        @Override public void onAuthenticationFailed(){ }
                        @Override public void onAuthenticationError(int code, CharSequence err){ js("window.bioResult&&window.bioResult(false)"); }
                    });
