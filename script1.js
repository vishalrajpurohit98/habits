...
var _todayCache='', _todayCacheAt=0;
function today(){ var n=Date.now(); if(n-_todayCacheAt<30000 && _todayCache) return _todayCache; _todayCache=fmt(new Date()); _todayCacheAt=n; return _todayCache; }
...
function normState(s){
  ...
  if(!(s.jrTpl instanceof Array)) s.jrTpl = [];
...
function renderAI(){
  ...
  try{ renderProactiveInsights(); }catch(e){}
}
...
function buildDataContext(query){
  ...
  // ===== JOURNAL (smart retrieval based on the question) =====
  if(state.jr && state.jr.length){
    var plain=function(h){var d=document.createElement('div');d.innerHTML=String(h||'');return (d.textContent||'').replace(/\s+/g,' ').trim();};
    var all=state.jr.slice().sort(function(a,b){return ((b.date||'')+(b.time||'')).localeCompare((a.date||'')+(a.time||''));});
    var chosen=[], q=(query||'').toLowerCase();
    var stop={what:1,when:1,that:1,this:1,about:1,between:1,happened:1,during:1,have:1,been:1,were:1,they:1,with:1,from:1,your:1,tell:1,show:1,give:1,days:1,day:1,entries:1,entry:1,journal:1,write:1,wrote:1,feeling:1,feel:1};
    var words=(q.match(/[a-z0-9']{4,}/g)||[]).filter(function(w){return !stop[w];});
    if(words.length){
      var scored=[];
      all.forEach(function(e){ var hay=(plain(e.content)+' '+(e.title||'')+' '+(e.mood||'')).toLowerCase(); var score=0; words.forEach(function(w){ if(hay.indexOf(w)>=0) score++; }); if(score>0) scored.push([score,e]); });
      scored.sort(function(a,b){return b[0]-a[0] || ((b[1].date||'')).localeCompare(a[1].date||'');});
      chosen=scored.slice(0,15).map(function(x){return x[1];});
    }
    var iso=q.match(/(\d{4}-\d{2}-\d{2})/g);
    if(iso&&iso.length){ var a=iso[0],b=iso[1]||iso[0]; if(a>b){var t=a;a=b;b=t;} all.forEach(function(e){ if(e.date>=a&&e.date<=b && chosen.indexOf(e)<0) chosen.push(e); }); }
    var moodWord={sad:'low',unhappy:'low',down:'low',low:'low',difficult:'diff',hard:'diff',happy:'good',great:'great',good:'good',okay:'okay'};
    Object.keys(moodWord).forEach(function(w){ if(q.indexOf(w)>=0){ all.forEach(function(e){ if(e.mood===moodWord[w] && chosen.indexOf(e)<0) chosen.push(e); }); } });
    all.slice(0,10).forEach(function(e){ if(chosen.indexOf(e)<0) chosen.push(e); });
    chosen=chosen.slice(0,30);
    chosen.sort(function(a,b){return ((b.date||'')+(b.time||'')).localeCompare((a.date||'')+(a.time||''));});
    lines.push('JOURNAL ('+state.jr.length+' entries total; '+chosen.length+' most relevant to the question shown):');
    chosen.forEach(function(e){var body=plain(e.content).slice(0,300);lines.push('- '+(e.date||'')+' '+(e.time||'')+(e.mood?' ['+e.mood+']':'')+(e.location?' @'+e.location:'')+' "'+String(e.title||'').replace(/"/g,'')+'": '+body);});
    lines.push('JOURNAL RULES: Answer journal questions from the entries above. If a specific entry is not shown but the total suggests more exist, say you searched the most relevant entries. Never invent journal history, dates, or counts.');
  } else { lines.push('JOURNAL: (no entries yet)'); }
  /* Cross-domain correlations (last 30 days) so the AI can answer relationship questions */
  try{ var ci=crossDomainInsights(); if(ci.length){ lines.push('CROSS-DOMAIN PATTERNS (last 30 days, computed from the user\u2019s data): '+ci.join(' ')); } }catch(e){}
  return lines.join('\n');
}
...
function renderAiPrefs(){
  ...
  if(p.responseStyle) items.push(['responseStyle','Response style: '+p.responseStyle]);
  if(p.responseLength) items.push(['responseLength','Response length: '+p.responseLength]);
  if(p.expenseAccount) items.push(['expenseAccount','Default expense account: '+p.expenseAccount]);
  if(p.expenseCategory) items.push(['expenseCategory','Default expense category: '+p.expenseCategory]);
  if(p.habitSchedule) items.push(['habitSchedule','Preferred habit schedule: '+p.habitSchedule]);
  if(Array.isArray(p.other)) p.other.forEach(function(o,i){ items.push(['other:'+i, o]); });
  if(!items.length){ box.innerHTML='<div class="setS" style="opacity:.7">Nothing saved yet.</div>'; return; }
  box.innerHTML=items.map(function(it){ return '<div class="aiPrefRow"><span>'+esc(it[1])+'</span><button class="aiPrefDel" data-pref="'+esc(it[0])+'" aria-label="Delete">✕</button></div>'; }).join('');
}
...
function uaiPrompt(text,conversationContext){
  ...
  +'For query actions, "message" must contain the FULL answer to the question (as many sentences as needed, with specific data from APP DATA). For create/update/delete actions, "message" is a short confirmation.\\n'
  +'MULTIPLE ACTIONS: If the user clearly asks for several actions at once (e.g. "add 3 habits: read, walk, meditate" or "log my mood and add a task"), return {"actions":[{"action":...,"params":{...}},{"action":...,"params":{...}}],"message":"short summary"}. Use the actions array ONLY for genuinely multiple distinct operations; for a single operation use the normal single-action object. Every step in the array must itself have all required fields; if any required field is missing, return a single clarify asking for it instead of a partial batch.\\n'
  +'FORMATTING: For query answers you may use markdown for readability: **bold**, "1." numbered lists, "- " bullets, and markdown tables (a header row of | cols |, then a |---|---| separator, then data rows). Use a table when presenting several rows of comparable numbers (e.g. weekly/monthly review, category breakdown). Keep tables compact.\\n\\n'
  +'ACTIONS:\n'
...
function uaiFormat(raw){
  ...
  // bold **x**
  s=s.replace(/\*\*(.+?)\*\*/g,'<b>$1</b>');
  // split a run-on numbered list ("... 1. a 2. b 3. c") onto separate lines
  s=s.replace(/\s(\d{1,2}\.\s)/g,'\n$1');
  // normalize existing newlines
  s=s.replace(/\r\n/g,'\n');
  var linesArr=s.split('\n');
  var htmlOut='', inOl=false, inUl=false;
  function closeLists(){ if(inOl){htmlOut+='</ol>';inOl=false;} if(inUl){htmlOut+='</ul>';inUl=false;} }
  for(var li2=0; li2<linesArr.length; li2++){
    var ln=linesArr[li2]; var t=ln.trim(); if(!t){ continue; }
    /* markdown table: current line has |, next line is a |---| separator */
    if(t.indexOf('|')>=0 && li2+1<linesArr.length && /^\|?\s*:?-{2,}/.test(linesArr[li2+1].trim().replace(/\|/g,'|'))){
      closeLists();
      var header=t.replace(/^\||\|$/g,'').split('|').map(function(c){return c.trim();});
      var rows=[]; var j=li2+2;
      while(j<linesArr.length && linesArr[j].indexOf('|')>=0){ rows.push(linesArr[j].trim().replace(/^\||\|$/g,'').split('|').map(function(c){return c.trim();})); j++; }
      var tb='<table class="uaiTable"><thead><tr>'+header.map(function(c){return '<th>'+c+'</th>';}).join('')+'</tr></thead><tbody>';
      rows.forEach(function(r){ tb+='<tr>'+r.map(function(c){return '<td>'+c+'</td>';}).join('')+'</tr>'; });
      tb+='</tbody></table>'; htmlOut+=tb; li2=j-1; continue;
    }
    var mOl=t.match(/^(\d{1,2})\.\s+(.*)$/);
    var mUl=t.match(/^[-•]\s+(.*)$/);
    if(mOl){ if(!inOl){closeLists();htmlOut+='<ol class="uaiList">';inOl=true;} htmlOut+='<li>'+mOl[2]+'</li>'; }
    else if(mUl){ if(!inUl){closeLists();htmlOut+='<ul class="uaiList">';inUl=true;} htmlOut+='<li>'+mUl[1]+'</li>'; }
    else { closeLists(); htmlOut+='<p class="uaiP">'+t+'</p>'; }
  }
  closeLists();
  // make journal dates (YYYY-MM-DD) tappable if that entry exists
  htmlOut=htmlOut.replace(/(\d{4}-\d{2}-\d{2})/g, function(d){
    var has=(state.jr||[]).some(function(e){return e.date===d;});
    return has ? '<a class="uaiJrLink" data-jrdate="'+d+'">'+d+'</a>' : d;
  });
  return htmlOut;
}
...
function uaiSend(text){
  ...
  gemCall(uaiPrompt(text,context),500).then(function(raw){
    ...
    if(_batch && _batch.length){
      /* If ANY step is incomplete, ask for that first (don't half-execute). */
      var incomplete=null;
      for(var bi=0; bi<_batch.length; bi++){
        var st=_batch[bi]||{}; if(!st.action) continue;
        if(st.action==='clarify' || st.action==='query') continue;
        var c=validateAIActionCompleteness(st.action, st.params||{});
        if(c){ incomplete=c; break; }
      }
      if(incomplete){
        var mh='<div class="uaiMsg bot">'+esc(incomplete.msg||'I need a bit more detail.')+'</div>';
        log.innerHTML+=mh; saveUaiChatTurn('assistant',incomplete.msg||''); log.scrollTop=log.scrollHeight; return;
      }
      /* Any destructive step needs confirm: if none confirmed, ask once for the whole batch. */
      var results=[], anyConfirm=false;
      _batch.forEach(function(st){
        if(!st||!st.action) return;
        if(st.action==='clarify'){ results.push({ok:0,msg:st.message||st.msg||''}); return; }
        try{
          var ex=executeAction({action:st.action, params:st.params||{}, message:st.message||st.msg||''});
          if(ex && ex.needConfirm) anyConfirm=true;
          results.push(ex||{ok:0,msg:'No result'});
        }catch(e){ results.push({ok:0,msg:'Could not complete: '+(e&&e.message||'error')}); }
      });
      var doneCount=results.filter(function(r){return r&&r.ok;}).length;
      var lines=results.map(function(r){ return (r&&r.ok?'✓ ':(r&&r.needConfirm?'⚠ ':'✕ '))+esc((r&&(r.detail||r.msg))||''); });
      var summary=(result.message?esc(result.message)+'<br>':'') + '<div class="uaiBatch">'+lines.join('<br>')+'</div>';
      log.innerHTML+='<div class="uaiMsg bot"><div class="uaiAction">'+summary+'</div></div>';
      saveUaiChatTurn('assistant', doneCount+' of '+results.length+' actions done');
      log.scrollTop=log.scrollHeight; reRenderCurrent();
      if(anyConfirm){ log.innerHTML+='<div class="uaiMsg bot" style="opacity:.75">Some steps need confirmation — please repeat those individually to confirm.</div>'; log.scrollTop=log.scrollHeight; }
      return;
    }
    ...
    if(completeness){
      ...
      if(result && result.action && result.action!=='clarify'){
        try{
          var _msgIn=result.message||result.msg||'';
          var _exec=executeAction({action:result.action, params:result.params||{}, message:_msgIn});
          if(_exec && typeof _exec==='object') result=_exec;
        }catch(e){ result={ok:0,msg:'Could not complete that: '+(e&&e.message||'error')}; }
      } else if(result && result.action==='clarify'){
        result={ok:0, msg:(result.message||result.msg||'Could you give me a bit more detail?'), isClarify:true};
      }
    }
...
function renderToday(){
  ...
  /* Momentum ring + "Best chain" removed from Today per user; keep #hero empty & hidden. */
  var _heroEl=$('hero'); if(_heroEl){ _heroEl.innerHTML=''; _heroEl.style.display='none'; }
  var rb = $('restBox');
  if(restDay){
    rb.style.display = '';
...
window.handleAndroidBack=function(){
  try{
    ...
    /* 2. open .sheet.open (mood, export, trash, etc.) */
    var openSheetEl=document.querySelector('.sheet.open'); if(openSheetEl){ if(typeof closeSheet==='function') closeSheet(); else openSheetEl.classList.remove('open'); return true; }
    /* 3. open menus/overlays */
    var menu=document.querySelector('.tcMenu.on,.jrAiMenu.on,.jrAskDayMenu.on'); if(menu){ menu.classList.remove('on'); return true; }
  ...
  return false; /* nothing to close -> let Android handle (exit) */
};
...
/* ===== Cross-domain insight + correlation engine (rule-based, no ML) ===== */
function _avg(a){ return a.length? a.reduce(function(s,x){return s+x;},0)/a.length : null; }
function habitCompletionRate(ds){
  var due=0,done=0;
  for(var i=0;i<state.habits.length;i++){ var h=state.habits[i]; if(h.arch)continue; if(dueOn(h,toDate(ds))){ due++; if(isDone(h,ds))done++; } }
  return due? done/due : null;
}
function sleepMinsOf(ds){ var s=(state.sleep||[]).find(function(x){return x.d===ds;}); return s&&s.mins?s.mins:null; }
function spendOf(ds){ var t=0,any=false; (state.tx||[]).forEach(function(x){ if(x.d===ds&&x.kind==='exp'){t+=(+x.amt||0);any=true;} }); return any?t:null; }
/* Pearson-ish: returns {r, n} or null */
function correlate(pairs){
  var xs=[],ys=[]; pairs.forEach(function(p){ if(p[0]!=null&&p[1]!=null){xs.push(p[0]);ys.push(p[1]);} });
  var n=xs.length; if(n<4) return null;
  var mx=_avg(xs),my=_avg(ys),num=0,dx=0,dy=0;
  for(var i=0;i<n;i++){ var a=xs[i]-mx,bb=ys[i]-my; num+=a*bb; dx+=a*a; dy+=bb*bb; }
  if(dx===0||dy===0) return null;
  return {r:num/Math.sqrt(dx*dy), n:n};
}
/* Build day-series for the last N days */
function _daySeries(days){
  days=days||30; var out=[]; var now=new Date();
  for(var i=0;i<days;i++){ var ds=fmt(addDays(now,-i)); out.push({ds:ds, mood:(moodOf(ds)>=0?moodOf(ds):null), sleep:sleepMinsOf(ds), habit:habitCompletionRate(ds), spend:spendOf(ds)}); }
  return out;
}
function crossDomainInsights(){
  var s=_daySeries(30), out=[];
  /* sleep vs mood (mood index: lower=better, so invert to "positivity") */
  var sm=correlate(s.map(function(d){return [d.sleep, d.mood!=null?(MOODS.length-1-d.mood):null];}));
  if(sm && Math.abs(sm.r)>=0.35){ out.push(sm.r>0? 'You tend to feel better on days you sleep more (based on '+sm.n+' days).' : 'More sleep hasn\u2019t lined up with better mood lately \u2014 something else may be affecting how you feel.'); }
  /* sleep vs habit completion */
  var sh=correlate(s.map(function(d){return [d.sleep, d.habit];}));
  if(sh && Math.abs(sh.r)>=0.35){ out.push(sh.r>0? 'Your habit completion is higher on days you sleep more.' : 'Your habits don\u2019t depend much on how much you slept.'); }
  /* mood vs spending */
  var ms=correlate(s.map(function(d){return [d.mood!=null?(MOODS.length-1-d.mood):null, d.spend];}));
  if(ms && Math.abs(ms.r)>=0.4){ out.push(ms.r<0? 'You tend to spend more on lower-mood days.' : 'Higher-mood days tend to come with a bit more spending.'); }
  return out;
}
/* Journal theme frequency + general proactive nudges */
function proactiveInsights(){
  var out=crossDomainInsights();
  /* journal recurring words (last 30 days) */
  try{
    var cutoff=fmt(addDays(new Date(),-30)), words={};
    (state.jr||[]).forEach(function(e){ if((e.date||'')>=cutoff){ var t=(function(h){var d=document.createElement('div');d.innerHTML=String(h||'');return (d.textContent||'').toLowerCase();})(e.content); (t.match(/[a-z']{4,}/g)||[]).forEach(function(w){ if(['that','this','with','have','been','were','they','from','your','about','really','would','could','still','which','there','their','what','when','just','like'].indexOf(w)<0) words[w]=(words[w]||0)+1; }); } });
    var top=Object.keys(words).filter(function(w){return words[w]>=4;}).sort(function(a,b){return words[b]-words[a];}).slice(0,1);
    if(top.length) out.push('You\u2019ve written about \u201C'+top[0]+'\u201D '+words[top[0]]+' times this month.');
  }catch(e){}
  /* streak-at-risk */
  try{ for(var i=0;i<state.habits.length;i++){ var h=state.habits[i]; if(h.arch)continue; var st=streak(h); if(st>=3 && dueOn(h,new Date()) && !isDone(h,today())){ out.push('Your '+st+'-day \u201C'+h.name+'\u201D streak is still open today \u2014 don\u2019t break the chain.'); break; } } }catch(e){}
...
function jrExportPdf(fromISO,toISO,tags){
  ...
  exportHtmlDoc(docHtml, jrExportTitle().replace(/[^a-z0-9]+/gi,'-').toLowerCase()||'journal');
}
...
function jrTemplateById(id){ var all=jrAllTemplates(); for(var i=0;i<all.length;i++) if(all[i].id===id) return all[i]; return null; }
function jrOpenTemplateEditor(){
  $('jrTplName').value='';
  $('jrTplBody').innerHTML='<p><br></p>';
  openSheet('jrTplSheet');
}
function jrSaveTemplate(){
  var name=($('jrTplName').value||'').trim().slice(0,60);
  var body=$('jrTplBody').innerHTML;
  if(!name){ $('jrTplName').focus(); toastN('Give the template a name'); return; }
  if(!jrStrip(body)){ $('jrTplBody').focus(); toastN('Add some template content'); return; }
  state.jrTpl=state.jrTpl||[];
  state.jrTpl.push({ id:'t'+Date.now().toString(36)+Math.random().toString(36).slice(2,6), icon:'\uD83D\uDCDD', name:name, body:body, createdAt:Date.now() });
  persist(); closeSheet(); jrRenderTpl(); toastN('Template saved');
}
function jrDeleteTemplate(id){
  var t=(state.jrTpl||[]).find(function(x){return x.id===id;});
  if(!t) return;
  if(!confirm('Delete the template "'+(t.name||'Custom')+'"?')) return;
  state.jrTpl=state.jrTpl.filter(function(x){return x.id!==id;});
  persist(); jrRenderTpl(); toastN('Template deleted');
}
/* Save the current entry's content as a reusable template. */
function jrSaveEntryAsTemplate(){
  var body=$('jrBody').innerHTML;
  if(!jrStrip(body)){ toastN('Nothing to save as a template'); return; }
  var name=prompt('Template name:', ($('jrTitle').value||'My template').slice(0,60));
  if(name===null) return;
  name=(name||'').trim().slice(0,60); if(!name){ toastN('Template needs a name'); return; }
  state.jrTpl=state.jrTpl||[];
  state.jrTpl.push({ id:'t'+Date.now().toString(36)+Math.random().toString(36).slice(2,6), icon:'\uD83D\uDCDD', name:name, body:body, createdAt:Date.now() });
  persist(); toastN('Saved as template'); jrRenderTpl();
}
...
/* ---- entry card ---- */
function jrClean(s){ return String(s||'')
  .replace(/\*\*([^*]+)\*\*/g,'$1').replace(/\*([^*]+)\*/g,'$1')   /* bold/italic */
  .replace(/^#{1,6}\s*/gm,'').replace(/`{1,3}/g,'')                /* headings / code ticks */
...
/* ---- master render ---- */
function jrWeekGoal(){ var g=(state.set&&state.set.jrWeekGoal); return (g&&g>=1&&g<=7)?g:0; }
function renderJrGoal(){
  var card=$('jrGoalCard'); if(!card) return;
  var goal=jrWeekGoal();
  if(!goal){ /* not set: show a subtle prompt */
    $('jrGoalLbl').textContent='Weekly writing goal';
    $('jrGoalFill').style.width='0%';
    $('jrGoalSub').textContent='Set a goal to build a writing habit.';
    return;
  }
  /* count distinct days journaled in the current week (Mon-Sun) */
  var now=new Date(), dow=(now.getDay()+6)%7; var monday=addDays(now,-dow);
  var days={}; (state.jr||[]).forEach(function(e){ if(!e.date) return; var d=toDate(e.date); if(d>=toDate(fmt(monday)) && d<=now) days[e.date]=1; });
  var have=Object.keys(days).length;
  var pct=Math.min(100, Math.round(have/goal*100));
  $('jrGoalLbl').textContent='Weekly writing goal';
  $('jrGoalFill').style.width=pct+'%';
  $('jrGoalSub').textContent = have>=goal ? ('✓ Goal met — '+have+'/'+goal+' days this week') : (have+' / '+goal+' days this week');
}
function renderJr(){
  if(!$('pgJr')) return;
...
function jrTimelineMore(){
  ...
  _jrTimelineShown=end;
  if(_jrTimelineShown<_jrTimelineFlat.length){
    list.insertAdjacentHTML('beforeend','<div class="jrTLsentinel" style="height:1px"></div>');
    var sent=list.querySelector('.jrTLsentinel');
    if('IntersectionObserver' in window){
      if(_jrTimelineIO){ try{_jrTimelineIO.disconnect();}catch(e){} }
      _jrTimelineIO=new IntersectionObserver(function(ents){ if(ents[0]&&ents[0].isIntersecting){ jrTimelineMore(); } },{rootMargin:'400px'});
      _jrTimelineIO.observe(sent);
    }
  } else if(_jrTimelineIO){ try{_jrTimelineIO.disconnect();}catch(e){} _jrTimelineIO=null; }
}
...
function openJr(id, prompt, tplBody, forceDate){
  ...
  var _jxe=$('jrExportEntryBtn'); if(_jxe) _jxe.style.display = id?'':'none';
  jrResetDel();
...
function jrEnsureEditable(){ var ed=$('jrBody'); if(ed){ ed.focus(); if(!ed.innerHTML.trim()){ ed.innerHTML='<p><br></p>'; jrPlaceCaretEnd(ed); } } }
function jrPlaceCaretEnd(el){ try{ var r=document.createRange(); r.selectNodeContents(el); r.collapse(false); var s=window.getSelection(); s.removeAllRanges(); s.addRange(r); }catch(e){} }
function jrExec(cmd, val){
  var ed=$('jrBody'); if(!ed) return;
  ed.focus();
  try{ document.execCommand('styleWithCSS', false, false); }catch(e){}
  try{ document.execCommand(cmd, false, val===undefined?null:val); }catch(e){}
  jrUpdTagLine();
}
function jrRt(c){ jrExec(c); }
function jrRtBlock(t){
  var ed=$('jrBody'); if(!ed) return; ed.focus();
  /* toggle: if the current block is already this type, revert to <p> */
  var cur='';
  try{ cur=(document.queryCommandValue('formatBlock')||'').toLowerCase(); }catch(e){}
  var target=(cur===t || cur==='<'+t+'>') ? 'p' : t;
  try{ document.execCommand('styleWithCSS', false, false); }catch(e){}
  try{ document.execCommand('formatBlock', false, '<'+target+'>'); }catch(e){ try{ document.execCommand('formatBlock', false, target); }catch(_){ } }
  jrUpdTagLine();
}
function jrRtList(kind){
  /* kind: insertUnorderedList | insertOrderedList — ensure a block exists first */
  var ed=$('jrBody'); if(!ed) return; ed.focus();
  if(!ed.innerHTML.trim()){ ed.innerHTML='<p><br></p>'; jrPlaceCaretEnd(ed); }
  try{ document.execCommand('styleWithCSS', false, false); }catch(e){}
  try{ document.execCommand(kind, false, null); }catch(e){}
  jrUpdTagLine();
}
function jrRtCheck(){
  var ed=$('jrBody'); if(!ed) return; ed.focus();
  try{ document.execCommand('insertHTML', false, '<div class="jrCheckLine">\u2610 </div>'); }catch(e){}
  jrUpdTagLine();
}
...
function _pickVoice(){
  try{
    ...
    var prefer=['Google','Natural','Samantha','Aria','Zira','Neural'];
    // prefer a high-quality voice in the user's language
    for(var pi=0; pi<prefer.length; pi++){ var v=vs.find(function(x){return x.lang&&x.lang.indexOf(lang.slice(0,2))===0 && (x.name||'').indexOf(prefer[pi])>=0;}); if(v) return v; }
    var vlang=vs.find(function(x){return x.lang&&x.lang.indexOf(lang.slice(0,2))===0;});
    return vlang||vs[0];
  }catch(e){ return null; }
}
...
function jrStopSpeak(){ try{ if(window.speechSynthesis) speechSynthesis.cancel(); }catch(e){} try{ if(nat&&nat.stopSpeak) nat.stopSpeak(); }catch(e){} }
...
/* ---- AI rewrite actions: generate a new version and let the user replace the entry ---- */
var _jrRewritePending=null;
function jrRewrite(kind){
  var bodyHtml=$('jrBody').innerHTML, body=jrStrip(bodyHtml), title=$('jrTitle').value.trim();
  if(!body){ jrAiShow('jrEntryAiOut', kind, 'Write something first, then I can '+kind.toLowerCase()+' it.'); return; }
  jrAiBox('jrEntryAiOut', kind);
  var instr={
    'Rewrite':'Rewrite this journal entry keeping the same meaning, first person, and the author\u2019s voice, but with clearer wording.',
    'Improve':'Improve the clarity, flow and grammar of this journal entry while preserving its meaning, first person and voice.',
    'Shorten':'Shorten this journal entry to about half its length, keeping the key points, first person and voice.',
    'Expand':'Expand this journal entry with a little more descriptive detail and reflection, staying truthful to what is written, in first person and the author\u2019s voice.',
    'Make professional':'Rewrite this journal entry in a more polished, professional tone, keeping the meaning and first person.',
    'Make personal':'Rewrite this journal entry in a warmer, more personal and reflective tone, keeping the meaning and first person.'
  }[kind]||'Rewrite this journal entry.';
  var prompt=instr+' Return ONLY the rewritten text as plain paragraphs (you may use blank lines between paragraphs). Do not add commentary, headings, or quotes.\n\nENTRY'+(title?' ("'+title+'")':'')+':\n'+body;
  gemCall(prompt, 700).then(function(r){
    r=String(r||'').trim();
    if(!r){ jrAiErr('jrEntryAiOut', kind, {message:'No rewrite returned.'}); return; }
    _jrRewritePending=jrTextToHtml(r);
    var preview=esc(r).replace(/\n{2,}/g,'<br><br>').replace(/\n/g,'<br>');
    var box=$('jrEntryAiOut');
    box.innerHTML='<div class="jrAiOut"><div class="h">'+esc(kind)+' \u00B7 preview</div><div class="b">'+preview+'</div>'
      +'<div class="jrSaveRow"><button class="primary" id="jrRwApply">Replace my entry</button><button class="ghost" id="jrRwCopy">Copy instead</button><button class="ghost" id="jrRwDiscard">Discard</button></div>'
      +'<div class="jrMut" style="text-align:left;padding:8px 2px 0">Your original stays until you tap Replace.</div></div>';
  }).catch(function(e){ jrAiErr('jrEntryAiOut', kind, e); });
}
function jrTextToHtml(t){
...
function jrInsertVoice(text){
  ...
  var ed=$('jrBody'); if(!ed) return;
  ed.focus();
  try{ document.execCommand('insertText', false, (ed.innerHTML.trim()&&!/(\s|>)$/.test(ed.innerHTML)?' ':'')+text); }
  catch(e){ ed.innerHTML += '<p>'+esc(text)+'</p>'; }
  jrUpdTagLine();
}
...
function jrJournalContext(query){
  var plain=function(h){var d=document.createElement('div');d.innerHTML=String(h||'');return (d.textContent||'').replace(/\s+/g,' ').trim();};
  var all=(state.jr||[]).slice().sort(function(a,b){return ((b.date||'')+(b.time||'')).localeCompare((a.date||'')+(a.time||''));});
  if(!all.length) return '(no journal entries yet)';
  var q=(query||'').toLowerCase(), chosen=[];
  var stop={what:1,when:1,that:1,this:1,about:1,between:1,happened:1,during:1,have:1,been:1,were:1,they:1,with:1,from:1,your:1,tell:1,show:1,give:1,days:1,day:1,entries:1,entry:1,journal:1,write:1,wrote:1,feeling:1,feel:1,make:1,makes:1,recurring:1};
  var words=(q.match(/[a-z0-9']{4,}/g)||[]).filter(function(w){return !stop[w];});
  if(words.length){var sc=[];all.forEach(function(e){var hay=(plain(e.content)+' '+(e.title||'')+' '+(e.mood||'')).toLowerCase();var s2=0;words.forEach(function(w){if(hay.indexOf(w)>=0)s2++;});if(s2>0)sc.push([s2,e]);});sc.sort(function(a,b){return b[0]-a[0];});chosen=sc.slice(0,18).map(function(x){return x[1];});}
  var iso=q.match(/(\d{4}-\d{2}-\d{2})/g);
  if(iso&&iso.length){var a=iso[0],b=iso[1]||iso[0];if(a>b){var t=a;a=b;b=t;}all.forEach(function(e){if(e.date>=a&&e.date<=b&&chosen.indexOf(e)<0)chosen.push(e);});}
  var mw={sad:'low',unhappy:'low',down:'low',low:'low',difficult:'diff',hard:'diff',happy:'good',great:'great',good:'good',okay:'okay'};
  Object.keys(mw).forEach(function(w){if(q.indexOf(w)>=0){all.forEach(function(e){if(e.mood===mw[w]&&chosen.indexOf(e)<0)chosen.push(e);});}});
  all.slice(0,12).forEach(function(e){if(chosen.indexOf(e)<0)chosen.push(e);});
  chosen=chosen.slice(0,35);
  chosen.sort(function(a,b){return ((b.date||'')+(b.time||'')).localeCompare((a.date||'')+(a.time||''));});
  var lines=['JOURNAL ('+all.length+' entries total; '+chosen.length+' most relevant shown):'];
...
function jrAskJournal(question){
  question=String(question||'').trim(); if(!question) return;
  var inp=$('jrAskInput'); if(inp) inp.value='';
  var log=$('jrChatLog'); if(!log) return;
  if(!(state.jr&&state.jr.length)){ jrSaveChatTurn('user',question); jrSaveChatTurn('assistant','Your journal is empty — write some entries first, then ask me anything about them.'); jrRenderChat(); return; }
  jrSaveChatTurn('user',question); jrRenderChat();
  log.innerHTML += '<div class="jrChatMsg bot" id="jrChatTyping" style="opacity:.55">Searching your journal…</div>'; log.scrollTop=log.scrollHeight;
  var convo=jrChatHistory().slice(-8).map(function(m){return (m.who==='user'?'User':'Assistant')+': '+m.text;}).join('\n');
  var prompt='You are a JOURNAL-ONLY conversational assistant. You may ONLY use the journal entries below to answer. You must NOT discuss habits, tasks, money, sleep, workouts, or anything outside the journal — if asked about those, briefly say you only handle journal questions. This is an ongoing conversation: use the recent turns to understand follow-ups (e.g. "tell me more", "what about last month"). Answer warmly and specifically, citing entry dates (YYYY-MM-DD). If the journal does not contain the answer, say you searched but could not find it. Never invent entries, dates, or counts. Use short paragraphs or a list when helpful.\n\n'+jrJournalContext(question+' '+convo)+'\n\nRECENT CONVERSATION:\n'+convo+'\n\nAnswer the latest user question using only the journal above.';
  gemCall(prompt, 600).then(function(r){
    var t=$('jrChatTyping'); if(t) t.remove();
    var ans=String(r||'').trim()||'I could not find anything about that in your journal.';
    jrSaveChatTurn('assistant',ans); jrRenderChat();
  }).catch(function(e){
    var t=$('jrChatTyping'); if(t) t.remove();
    var msg=(e&&e.message)||'';
    var out=/no api key|api key|unknown provider/i.test(msg) ? '\u26A0 Add a free API key in More \u2192 AI settings to enable journal answers.' : 'Could not get an answer: '+msg;
    jrSaveChatTurn('assistant',out); jrRenderChat();
  });
}
function jrWholeAi(){
...
var webTimers = [];
/* Shared PDF/report delivery. Web: open + print (real PDF via browser). Android: native
   printHtmlToPdf if available (real PDF), else save the HTML so the export never silently fails. */
function exportHtmlDoc(html, baseName){
  baseName = baseName || ('report-'+today());
  // Android WebView path
  if(typeof nat!=='undefined' && nat){
    try{
      if(nat.printHtmlToPdf){ var okp=nat.printHtmlToPdf(baseName+'.pdf', html); if(okp!==false){ toastN('Preparing PDF\u2026'); return true; } }
    }catch(e){}
    // fallback: save HTML (user opens/print-to-PDF from a browser)
    try{
      var b64=btoa(unescape(encodeURIComponent(html)));
      var p=nat.saveFile(baseName+'.html','text/html',b64);
      toastN(p?('Saved '+baseName+'.html to '+p):'Saved '+baseName+'.html');
      return true;
    }catch(e2){ toastN('Could not export'); return false; }
  }
  // Web path: open + print
  var w=window.open('','_blank');
  if(w){ w.document.write(html); w.document.close(); setTimeout(function(){ try{w.print();}catch(e){} },450); return true; }
  toastN('Allow pop-ups to export the PDF, or use Excel/CSV');
  return false;
}
...
function exportExLogPdf(ex){
  ...
  var doc2='<html><head><meta charset="utf-8"><title>'+esc(ex.name)+' \u2014 Workout Log</title><style>'
    ...
    +'</body></html>';
  exportHtmlDoc(doc2, 'expense-report');
}
...
function renderExp(){
  ...
  var heroAcct=(state.set&&state.set.heroAcct)||'all';
  var accs=activeAccts();
  if(heroAcct!=='all' && !acctById(heroAcct)) heroAcct='all';
...
function exportExpPdf(){
  ...
  var doc2='<html><head><meta charset="utf-8"><title>'+title+'</title><style>'
    ...
    +'</body></html>';
  exportHtmlDoc(doc2, 'expense-log');
}
...
function renderSyncIcon(){
  var dot=$('syncIconDot'), ic=$('syncIcon'); if(!dot) return;
  var configured=(typeof syncEnabled==='function')?syncEnabled():false;
  var signedIn=(typeof fbUser!=='undefined')&&!!fbUser;
  var online=(typeof isOnline==='function')?isOnline():true;
  var st, t, sub;
  if(!configured){ st='offline'; t='Sync not set up'; sub='Enable cloud sync in Settings to back up across devices.'; }
  else if(!signedIn){ st='offline'; t='Not signed in'; sub='Sign in from Settings to sync.'; }
  else if(!online){ st='offline'; t='Offline'; sub='Changes are saved locally and will sync when you\u2019re back online.'; }
  else if(syncState==='syncing'){ st='syncing'; t='Syncing…'; sub='Updating your data with the cloud.'; }
  else if(syncState==='error'){ st='error'; t='Sync error'; sub='Could not reach the cloud. Tap Sync now to retry.'; }
  else { st='synced'; t='Synced'; sub='Your data is backed up and up to date.'; }
  dot.className='syncIconDot '+st;
  if(ic) ic.classList.toggle('spin', st==='syncing');
  var pt=$('syncPopT'), ps=$('syncPopS'), pd=$('syncPopDot');
  if(pt) pt.textContent=t; if(ps) ps.textContent=sub; if(pd) pd.className='syncPopDot '+st;
}
  ...
  return loadFirestoreSDK().then(function(){
    ...
    if(!fbApp){
      ...
      return fbFS.enablePersistence({synchronizeTabs:true}).catch(function(e){
        /* Persistence is OPTIONAL — Firestore works fully online without it. In some WebViews IndexedDB
           is blocked/throws; swallow ALL persistence errors so init always proceeds to auth wiring. */
        try{ console.warn('Firestore persistence unavailable:', e&&e.code); }catch(_){}
        return;
      }).then(function(){
...
function pushRecordSync(force){if(syncLegacyMode)return pushLegacySync(force);if(!fbRecords||!fbUser)return Promise.resolve();if(!isOnline()&&!force){setSyncState('cached');return Promise.resolve();}queueChangedSyncRecords();var keys=Object.keys(syncPendingRecords);if(!keys.length){setSyncState('synced');return writeSyncMeta('synced').then(function(){setSyncState('synced');});}setSyncState('syncing');
  ...
  function commitFrom(i){
    ...
    return batch.commit().then(function(){
      ...
      window._syncProgressAt=Date.now();   /* heartbeat: sync is making progress */
...
/* ================= native sync & alarms ================= */
/* Adaptive reminder timing: learns your usual journaling time from entry timestamps.
   This is a lightweight heuristic (median of recent entry times), NOT a trained ML model. */
function adaptiveJournalTime(fallback){
  try{
    var cutoff=fmt(addDays(new Date(),-30)), mins=[];
    (state.jr||[]).forEach(function(e){ if((e.date||'')>=cutoff && /^\d{2}:\d{2}$/.test(e.time||'')){ var p=e.time.split(':'); mins.push((+p[0])*60+(+p[1])); } });
    if(mins.length<5) return fallback||'21:00';         /* not enough signal yet */
    mins.sort(function(a,b){return a-b;});
    var med=mins[Math.floor(mins.length/2)];
    /* nudge ~30 min before the usual time, clamped to a sane evening-ish window */
    var t=med-30; if(t<6*60) t=med; /* if very early, use the median itself */
    var hh=Math.floor(t/60), mm=t%60;
    return (hh<10?'0':'')+hh+':'+(mm<10?'0':'')+mm;
  }catch(e){ return fallback||'21:00'; }
}
function adaptiveInsightForReminder(){
  /* e.g. detect weekend workout misses -> caller can use for smarter copy */
  try{
    var wk=0,wkMiss=0;
    for(var i=0;i<28;i++){ var d=addDays(new Date(),-i), dow=d.getDay(); if(dow===0||dow===6){ for(var hi=0;hi<state.habits.length;hi++){ var h=state.habits[hi]; if(h.arch)continue; if(/work ?out|gym|exercise|run/i.test(h.name)&&dueOn(h,d)){ wk++; if(!isDone(h,fmt(d)))wkMiss++; } } } }
    if(wk>=4 && wkMiss/wk>0.6) return 'weekend-workout';
  }catch(e){}
  return '';
}
function computeAlarms(){
  ...
  var _jrTime = (rc.adaptive!==false) ? adaptiveJournalTime(rc.journalTime||'21:00') : (rc.journalTime||'21:00');
  dailyRem(jrOn, _jrTime, 'journal-rem', '\uD83D\uDCD3', 'Journal time', 'Take a moment to write about your day');
  ...
  // Overdue nudges: any incomplete task past due gets a morning reminder for the next few days.
  if(!rc || rc.overdue!==false){ for(var oi=0;oi<state.tasks.length;oi++){var ot=state.tasks[oi];if(taskEffectiveStatus(ot)!=='overdue')continue;for(var od=0;od<3;od++){var day=new Date(now+od*86400000);var at2=new Date(day.getFullYear(),day.getMonth(),day.getDate(),9,0,0,0);if(at2.getTime()<=now||at2.getTime()>now+3*86400000)continue;out.push({c:intHash('task-overdue|'+ot.id+'|'+fmt(at2)),t:at2.getTime(),h:'',task:ot.id,n:'⏰ Overdue: '+ot.title,e:'🔴',b:'This task was due '+niceDate(ot.dueDate)+' — still open',r:0});}}}
  out.sort(function(a,b){ return a.t - b.t; });
  return JSON.stringify(out.slice(0, 140));
}
function pushPinnedTasksNotif(){
  if(!nat || !nat.showPinnedTasks) return;
  try{
    var pins=(state.tasks||[]).filter(function(t){ return t.pinned && taskEffectiveStatus(t)!=='completed'; })
      .slice(0,6).map(function(t){ return {id:t.id, title:t.title, due:(t.dueDate?niceDate(t.dueDate):'')}; });
    if(pins.length) nat.showPinnedTasks(JSON.stringify(pins));
    else if(nat.hidePinnedTasks) nat.hidePinnedTasks();
  }catch(e){}
}
function pushAlarms(){
...
function haptic(){ try{ if(typeof nat!=="undefined"&&nat&&nat.haptic) nat.haptic(); else if(navigator.vibrate) navigator.vibrate(15); }catch(e){} }
...
function init(){
  ...
  var _tplSave=$('jrTplSave'); if(_tplSave) _tplSave.addEventListener('click', jrSaveTemplate);
  var _tplTb=$('jrTplToolbar'); if(_tplTb){ _tplTb.addEventListener('mousedown',function(e){e.preventDefault();}); _tplTb.addEventListener('click', function(e){
  ...
  (function(){
    ...
    if(qtog){
      qtog.classList.toggle('on', !!rc.quickAdd);
      if(rc.quickAdd && nat && nat.showQuickAdd){ try{ nat.showQuickAdd(); }catch(e){} }
      qtog.addEventListener('click',function(){
        var r=state.set.reminders; r.quickAdd=!r.quickAdd; this.classList.toggle('on',r.quickAdd); persist();
        try{ if(r.quickAdd){ if(nat&&nat.showQuickAdd) nat.showQuickAdd(); else toastN('Available in the Android app'); } else { if(nat&&nat.hideQuickAdd) nat.hideQuickAdd(); } }catch(e){}
      });
    }
  })();
  /* ===== Stats sub-tabs ===== */
  ...
  (function(){
    ...
    var MONK=(function(){
      var W='#FFFFFF',OFF='#EDE7DB',CREAM='#F3ECDD',CHAR='#2A2A2E',
          NAVY='#1E2A5E',TEAL='#12B5A5',EMER='#0FA36B',INDIGO='#4F6BF0',GOLD='#F5A623',
          SUN='#FF9F40',BLUE='#3E7BFA',LAV='#9B7BFF',GOLDM='#C9A24B';
      function fig(color,scale,cx,cy){scale=scale||1;cx=cx||12;cy=cy||12;
        return '<g transform="translate('+cx+' '+cy+') scale('+scale+') translate(-12 -12)" fill="'+color+'">'
          +'<circle cx="12" cy="6.4" r="2.5"/>'
          +'<path d="M12 9c-2.4 0-4 1.8-4.4 4.2-.15.9.5 1.6 1.4 1.6h6c.9 0 1.55-.7 1.4-1.6C16 10.8 14.4 9 12 9z"/>'
          +'<path d="M5.5 15.4c-.9 0-1.6.9-1.2 1.7.9 1.9 4 3.2 7.7 3.2s6.8-1.3 7.7-3.2c.4-.8-.3-1.7-1.2-1.7H5.5z"/>'
          +'</g>';}
      return {
        minimal:{bg:NAVY, inner:function(){return fig(W,1,12,12.2);}},
        progress:{bg:EMER, inner:function(){return '<path d="M17.5 9.5l3-3M20.5 6.5h-2.6M20.5 6.5v2.6" stroke="'+W+'" stroke-width="1.6" fill="none" stroke-linecap="round" stroke-linejoin="round"/><path d="M6 12c1.5-2.5 4-4 7.5-5" stroke="'+W+'" stroke-width="1.4" fill="none" stroke-linecap="round" opacity=".55"/>'+fig(W,.92,11,13);}},
        ring:{bg:NAVY, inner:function(){return '<circle cx="12" cy="12" r="9.4" fill="none" stroke="'+TEAL+'" stroke-width="1.8" stroke-linecap="round" stroke-dasharray="52 8" transform="rotate(-90 12 12)"/>'+fig(OFF,.78,12,12.4);}},
        check:{bg:INDIGO, inner:function(){return fig(W,.95,12,11.6)+'<path d="M8.4 18.2l2.4 2.4 5-5.2" stroke="'+GOLD+'" stroke-width="1.9" fill="none" stroke-linecap="round" stroke-linejoin="round"/>';}},
        sun:{bg:CHAR, inner:function(){return '<circle cx="12" cy="12.5" r="6" fill="'+SUN+'"/><g stroke="'+GOLD+'" stroke-width="1.5" stroke-linecap="round"><path d="M12 3.2v2.2M4.6 6.6l1.5 1.5M19.4 6.6l-1.5 1.5M2.4 13h2M19.6 13h2"/></g>'+fig(CHAR,.8,12,13.4);}},
        mountain:{bg:'#123A63', inner:function(){return '<path d="M2 20l6-8 3.5 4.2L15 10l7 10z" fill="'+BLUE+'" opacity=".5"/><path d="M2 20l6-8 3.5 4.2" fill="none" stroke="#9CC4FF" stroke-width="1" opacity=".5"/>'+fig(W,.7,12,14.4);}},
        mind:{bg:INDIGO, inner:function(){return '<path d="M12 2.6c4.4 0 7.4 3 7.4 6.2 0 2-1.2 3.4-1.2 3.4M12 2.6c-4.4 0-7.4 3-7.4 6.2 0 2 1.2 3.4 1.2 3.4" fill="none" stroke="'+LAV+'" stroke-width="1.7" stroke-linecap="round"/>'+fig(W,.8,12,13);}},
        orbit:{bg:NAVY, inner:function(){return '<ellipse cx="12" cy="12" rx="10" ry="6" fill="none" stroke="'+TEAL+'" stroke-width="1.3" opacity=".7" transform="rotate(-24 12 12)"/><ellipse cx="12" cy="12" rx="10" ry="6" fill="none" stroke="'+BLUE+'" stroke-width="1.3" opacity=".55" transform="rotate(30 12 12)"/><circle cx="21" cy="10.5" r="1.3" fill="'+TEAL+'"/><circle cx="4" cy="14" r="1.1" fill="'+BLUE+'"/>'+fig(W,.66,12,12.4);}},
        geometric:{bg:'#0E4A45', inner:function(){return '<circle cx="12" cy="6.6" r="2.6" fill="'+CREAM+'"/><rect x="8" y="9.4" width="8" height="6" rx="3.2" fill="'+CREAM+'"/><rect x="4.5" y="15" width="15" height="4.4" rx="2.2" fill="'+TEAL+'"/>';}},
        premium:{bg:CHAR, inner:function(){return '<path d="M12 2.2c5 3.5 7.6 7 7.6 11.2A7.6 7.6 0 014.4 13.4C4.4 9.2 7 5.7 12 2.2z" fill="'+GOLDM+'"/><circle cx="12" cy="10.6" r="2.1" fill="'+CHAR+'"/><path d="M12 13c-2 0-3.4 1.5-3.7 3.4-.12.8.45 1.4 1.2 1.4h5c.75 0 1.32-.6 1.2-1.4C15.4 14.5 14 13 12 13z" fill="'+CHAR+'"/>';}}
      };
    })();
    var ORDER=['minimal','progress','ring','check','sun','mountain','mind','orbit','geometric','premium'];
    function svg(id, cls){ var c=MONK[id]||MONK.minimal; return '<svg class="'+(cls||'')+'" viewBox="0 0 24 24"><rect x="0" y="0" width="24" height="24" rx="6" fill="'+c.bg+'"/>'+c.inner()+'</svg>'; }
    function current(){ var v=(state.set&&state.set.appIcon)||'orbit'; return MONK[v]?v:'orbit'; } /* migrate old ids -> orbit */
    function applyIcon(id){
      /* The in-app UI keeps the simple 🧠 brand; the picker only records which branded
         logo to use for the EXTERNAL app icon (home-screen/favicon/PWA). We do not overwrite
         the in-app navLogo. Elements explicitly tagged data-brandicon (none by default) still mirror. */
      var mk=svg(id,'brandSvg');
      document.querySelectorAll('[data-brandicon]').forEach(function(el){ el.innerHTML=mk; });
    }
    function render(){
      if(!grid) return;
      var c=current();
      grid.innerHTML=ORDER.map(function(id){ return '<button class="iconPick'+(id===c?' sel':'')+'" data-icon="'+id+'" aria-label="'+id+'">'+svg(id)+'</button>'; }).join('');
    }
    if(grid){
      render();
      grid.addEventListener('click', function(e){
        var b=e.target.closest('[data-icon]'); if(!b) return;
        var id=b.getAttribute('data-icon');
        state.set=state.set||{}; state.set.appIcon=id; persist();
        render(); applyIcon(id); if(typeof toastN==='function') toastN('App icon updated');
      });
    }
    try{ applyIcon(current()); }catch(e){}
    window._applyAppIcon=applyIcon; window._currentAppIcon=current;
  })();
  /* ===== Sync status icon ===== */
  (function(){
    var ic=$('syncIcon'), pop=$('syncPop'); if(!ic||!pop) return;
    try{ renderSyncIcon(); }catch(e){}
    ic.addEventListener('click', function(e){ e.stopPropagation(); try{renderSyncIcon();}catch(_){} pop.classList.toggle('on'); });
    var pb=$('syncPopBtn'); if(pb) pb.addEventListener('click', function(){ pop.classList.remove('on'); try{ if(typeof pullThenPush==='function') pullThenPush(); else if(typeof doSync==='function') doSync(); else if(typeof pushRecordSync==='function') pushRecordSync(false); }catch(e){} toastN('Syncing…'); });
    document.addEventListener('click', function(e){ if(pop.classList.contains('on') && !pop.contains(e.target) && e.target!==ic && !ic.contains(e.target)) pop.classList.remove('on'); });
    /* refresh icon when connectivity changes */
  ...
  var _vaultPin=null, _vaultData={pw:[],note:[]}, _vaultFilter='all', _vaultSort='upd', _vaultQuery='', _vaultDetail=null, _vaultEditIdx=-1, _vaultEditType='pw', _vaultCatFilter='', _vaultTagFilter='';
  var VAULT_DEFAULT_CATS=['Personal','Work','Finance','Travel','Shopping','Documents','Other'];
  function allVaultCats(){ var custom=(state.vaultCats||[]); var seen={}, out=[]; VAULT_DEFAULT_CATS.concat(custom).forEach(function(c){ c=String(c||'').trim(); if(c&&!seen[c.toLowerCase()]){ seen[c.toLowerCase()]=1; out.push(c); } }); return out; }
  function addVaultCat(name){ name=String(name||'').trim(); if(!name) return false; if(allVaultCats().some(function(c){return c.toLowerCase()===name.toLowerCase();})) return false; state.vaultCats=state.vaultCats||[]; state.vaultCats.push(name); persist(); return true; }
  ...
  function vaultExportExcel(){
    ...
    var wb=XLSX.utils.book_new();
    var cats=allVaultCats();
    var pwWs=XLSX.utils.aoa_to_sheet(pwRows);
    var noteWs=XLSX.utils.aoa_to_sheet(noteRows);
    /* Restrict Category column to existing categories via a dropdown data-validation.
       Passwords: category = column E (index 4). Notes: category = column C (index 2). */
    var listFormula='"'+cats.join(',')+'"';
    function addCatDV(ws, colLetter, maxRow){ ws['!dataValidation']=ws['!dataValidation']||[]; ws['!dataValidation'].push({ sqref:colLetter+'2:'+colLetter+(maxRow), type:'list', formula1:listFormula, allowBlank:true, showDropDown:true }); }
    try{ addCatDV(pwWs,'E', pwRows.length+50); addCatDV(noteWs,'C', noteRows.length+50); }catch(e){}
    XLSX.utils.book_append_sheet(wb, pwWs, 'Passwords');
    XLSX.utils.book_append_sheet(wb, noteWs, 'Notes');
    /* Reference sheet listing valid categories (so Excel/Sheets users see the allowed set even if
      ...
      try{
        ...
        if(wb.SheetNames.indexOf('Passwords')>=0){
          ...
          for(var i=1;i<rows.length;i++){ var r=rows[i]; if(!r||!String(r[0]||'').trim()) continue;
            var obj={ title:String(r[0]).trim(), user:String(r[1]||'').trim(), pass:String(r[2]||''), url:String(r[3]||'').trim(), category:String(r[4]||'Other').trim()||'Other', tags:parseTags(r[5]), notes:String(r[6]||'').trim(), favorite:/^y/i.test(String(r[7]||'')), extra:strToExtra(r[8]) }; addVaultCat(obj.category);
        ...
        if(wb.SheetNames.indexOf('Notes')>=0){
          ...
          for(var j=1;j<nr.length;j++){ var n=nr[j]; if(!n||!String(n[0]||'').trim()) continue;
            var no={ title:String(n[0]).trim(), body:String(n[1]||''), category:String(n[2]||'Other').trim()||'Other', tags:parseTags(n[3]), favorite:/^y/i.test(String(n[4]||'')) }; addVaultCat(no.category);
  ...
  function vaultMatch(entry, q){ if(!q) return true; q=q.toLowerCase(); var it=entry.it;
    var extraStr=(it.extra||[]).map(function(e){return (e.label||'')+' '+(e.value||'');}).join(' ');
    var hay=[it.title,it.user,it.url,it.body,it.notes,it.category,(it.tags||[]).join(' '),extraStr].join(' ').toLowerCase();
    return hay.indexOf(q)>=0; }
  function renderCatTagChips(){
    var box=$('vaultCatChips'); if(!box) return;
    var cats={}, tags={};
    vaultAllItems().forEach(function(e){ if(e.it.category) cats[e.it.category]=(cats[e.it.category]||0)+1; (e.it.tags||[]).forEach(function(t){ tags[t]=(tags[t]||0)+1; }); });
    var html='';
    Object.keys(cats).sort().forEach(function(c){ html+='<button class="vChip cat'+(_vaultCatFilter===c?' on':'')+'" data-vcat="'+esc(c)+'">'+esc(c)+' ('+cats[c]+')</button>'; });
    Object.keys(tags).sort().forEach(function(t){ html+='<button class="vChip tag'+(_vaultTagFilter===t?' on':'')+'" data-vtag="'+esc(t)+'">#'+esc(t)+'</button>'; });
  ...
  function renderVault(){
    ...
    if(_vaultCatFilter) items=items.filter(function(e){ return e.it.category===_vaultCatFilter; });
    if(_vaultTagFilter) items=items.filter(function(e){ return (e.it.tags||[]).indexOf(_vaultTagFilter)>=0; });
  ...
  function fillCatSelect(sel, val){ if(!sel) return; var cats=allVaultCats(); if(val&&cats.indexOf(val)<0) cats.push(val); sel.innerHTML=cats.map(function(c){return '<option'+(c===val?' selected':'')+'>'+esc(c)+'</option>';}).join('')+'<option value="__new">＋ New category…</option>'; sel.onchange=function(){ if(this.value==='__new'){ var nm=prompt('New category name'); if(nm&&addVaultCat(nm)){ fillCatSelect(sel, nm.trim()); } else { fillCatSelect(sel, cats[0]); } } }; }
  /* ---- forms ---- */
  ...
  if(_vSheet){
    ...
    _vSheet.addEventListener('click', function(e){
      ...
      var catc=e.target.closest('[data-vcat]'); if(catc){ var c=catc.getAttribute('data-vcat'); _vaultCatFilter=(_vaultCatFilter===c?'':c); renderVault(); return; }
      var tagc=e.target.closest('[data-vtag]'); if(tagc){ var t=tagc.getAttribute('data-vtag'); _vaultTagFilter=(_vaultTagFilter===t?'':t); renderVault(); return; }
  ...
  (function(){
    ...
    if(tx) tx.addEventListener('click', function(e){
      ...
      var id=row.getAttribute('data-tx'); if(txSel.has(id)) txSel.delete(id); else txSel.add(id); paint();
    }, true);
    if(del) del.addEventListener('click', function(){
      if(!txSel.size) return;
      if(!confirm('Delete '+txSel.size+' transaction'+(txSel.size>1?'s':'')+'? This cannot be undone.')) return;
      state.tx=state.tx.filter(function(t){ return !txSel.has(t.id); });
      persist(); txSel.clear(); txMode=false; paint(); renderExp(); toastN('Transactions deleted');
    });
    ...
    if(jl) jl.addEventListener('click', function(e){
      ...
      e.stopPropagation(); e.preventDefault();
      var id=card.getAttribute('data-jid'); if(jSel.has(id)) jSel.delete(id); else jSel.add(id); jpaint();
    }, true);
  ...
  /* ===== AI preferences viewer ===== */
  try{ renderAiPrefs(); }catch(e){}
  var _prefBox=$('aiPrefsList');
  if(_prefBox) _prefBox.addEventListener('click', function(e){
    var b=e.target.closest('[data-pref]'); if(!b) return;
    var key=b.getAttribute('data-pref'); var p=getAiPreferences();
    if(key.indexOf('other:')===0){ var idx=+key.split(':')[1]; if(Array.isArray(p.other)) p.other.splice(idx,1); }
    else { delete p[key]; }
    saveAiPreferences(p); toastN('Preference removed');
  });
  ...
  $('jrTplGrid').addEventListener('click', function(e){
    var del=climb(e.target,this,'data-jtpldel'); if(del){ e.stopPropagation(); jrDeleteTemplate(del.getAttribute('data-jtpldel')); return; }
  ...
  var _jrAskSend=$('jrAskSend'); if(_jrAskSend) _jrAskSend.addEventListener('click', function(){ var inp=$('jrAskInput'); if(inp&&inp.value.trim()) jrAskJournal(inp.value); });
  var _jrAskInput=$('jrAskInput'); if(_jrAskInput) _jrAskInput.addEventListener('keydown', function(e){ if(e.key==='Enter'&&this.value.trim()) jrAskJournal(this.value); });
  var _jrAskChips=$('jrAskChips'); if(_jrAskChips) _jrAskChips.addEventListener('click', function(e){ var b=climb(e.target,this,'data-jrask'); if(b){ var qv=b.getAttribute('data-jrask'); jrAskJournal(qv); } });
  var _jrChatClear=$('jrChatClear'); if(_jrChatClear) _jrChatClear.addEventListener('click', jrClearChat);
  ...
  $('jrToolbar').addEventListener('mousedown', function(e){ e.preventDefault(); }); /* keep editor selection */
  $('jrToolbar').addEventListener('click', function(e){
    var b=climb(e.target,this,'data-jrt');
    if(b){ var c=b.getAttribute('data-jrt'); if(c==='insertUnorderedList'||c==='insertOrderedList') jrRtList(c); else jrRt(c); return; }
    b=climb(e.target,this,'data-jrtblock'); if(b){ jrRtBlock(b.getAttribute('data-jrtblock')); return; }
    b=climb(e.target,this,'data-jrtcheck'); if(b){ jrRtCheck(); return; }
    b=climb(e.target,this,'data-jrmic'); if(b){ jrToggleVoice(); }
  });
  ...
  $('jrAiMenu').addEventListener('click', function(e){
    var b=climb(e.target,this,'data-jrai'); if(b){ jrEntryAi(b.getAttribute('data-jrai')); return; }
  ...
  var _jgoal=$('jrGoalSet'); if(_jgoal) _jgoal.addEventListener('click', function(){
    var cur=jrWeekGoal()||3;
    var v=prompt('How many days per week do you want to journal? (1-7)', String(cur));
    if(v===null) return;
    var n=Math.max(0,Math.min(7,parseInt(v,10)||0));
    state.set=state.set||{}; state.set.jrWeekGoal=n; persist(); renderJrGoal();
    toastN(n?('Goal set: '+n+' days/week'):'Goal cleared');
  });
  $('jrDelBtn').addEventListener('click', onJrDelete);
  var _jxeBtn=$('jrExportEntryBtn'); if(_jxeBtn) _jxeBtn.addEventListener('click', function(){
    if(!jrEd || !jrEd.id) { toastN('Save the entry first'); return; }
    var live=(state.jr||[]).find(function(e){return e.id===jrEd.id;})||jrEd;
    try{ var html=jrExportDocHtml([live], (live.title||('Journal '+live.date))); exportHtmlDoc(html, 'journal-'+(live.date||today())); }
    catch(e){ toastN('Could not export entry'); }
  });
  ...
  var _dupH=$('dupHabitBtn'); if(_dupH) _dupH.addEventListener('click', function(){
    var h=findHabit(detailId); if(!h) return;
    var copy=JSON.parse(JSON.stringify(h));
    copy.id='h'+Date.now().toString(36)+Math.random().toString(36).slice(2,7);
    copy.name=(h.name+' (copy)').slice(0,30);
    copy.done={}; copy.frozen={}; copy.created=today(); copy.start=today(); copy.arch=false; copy.archAt='';
    state.habits.push(normHabit(copy)); persist();
    closeSheet(); renderToday(); toastN('Habit duplicated');
  });
  ...
  $('uaiLog').addEventListener('click',function(e){
    var b=e.target.closest('[data-followup]');if(!b)return;$('uaiInput').value=b.getAttribute('data-followup');doUaiSend();
  });
