...
function jrAskJournal(question){
  ...
  var box=$('jrAskOut'); if(!box) return;
  if(!(state.jr&&state.jr.length)){ box.innerHTML='<div class="jrAiOut"><div class="b">Your journal is empty — write some entries first.</div></div>'; return; }
  box.innerHTML='<div class="jrAiOut"><div class="h">\u2728 '+esc(question)+' <span class="jrSpin"></span></div><div class="b">Searching your journal…</div></div>';
  var prompt='You are a JOURNAL-ONLY assistant. You may ONLY use the journal entries below to answer. You must NOT discuss habits, tasks, money, sleep, workouts, or anything outside the journal — if asked about those, say you only handle journal questions. Answer the user\'s question warmly and specifically, citing entry dates. If the journal does not contain the answer, say you searched but could not find it. Never invent entries, dates, or counts.\n\n'+jrJournalContext(question)+'\n\nQUESTION: '+question+'\n\nAnswer in a few sentences using only the journal above.';
  gemCall(prompt, 500).then(function(r){
    var safe=String(r||'').replace(/</g,'&lt;').replace(/\*\*(.*?)\*\*/g,'<b>$1</b>').replace(/\n/g,'<br>');
    box.innerHTML='<div class="jrAiOut"><div class="h">\u2728 '+esc(question)+'</div><div class="b">'+safe+'</div><button class="jrListen" data-jrtts="'+esc(String(r||'')).replace(/"/g,'&quot;')+'">\uD83D\uDD0A Listen</button></div>';
  }).catch(function(e){
    var msg=(e&&e.message)||'';
    if(/no api key|api key|unknown provider/i.test(msg)) box.innerHTML='<div class="jrAiOut"><div class="b">\u26A0 Add a free API key in More \u2192 AI settings to enable journal answers.</div></div>';
    else box.innerHTML='<div class="jrAiOut"><div class="b">Could not get an answer: '+esc(msg)+'</div></div>';
  });
}
function jrWholeAi(){
...
function init(){
  ...
  var _jrAskChips=$('jrAskChips'); if(_jrAskChips) _jrAskChips.addEventListener('click', function(e){ var b=climb(e.target,this,'data-jrask'); if(b){ var qv=b.getAttribute('data-jrask'); var inp=$('jrAskInput'); if(inp) inp.value=qv; jrAskJournal(qv); } });
